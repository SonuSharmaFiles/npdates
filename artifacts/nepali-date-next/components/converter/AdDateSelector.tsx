...
export function AdDateSelector({ date, onChange }: AdDateSelectorProps) {
  ...
  useEffect(() => {
    ...
    setDaysInMonth(days);
    if (date.day > days) onChange({ ...date, day: days });
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      <div className="space-y-2">
        ...
        <Select value={date.year.toString()} onValueChange={(v) => onChange({ ...date, year: parseInt(v) })}>
      ...
      <div className="space-y-2">
        ...
        <Select value={date.month.toString()} onValueChange={(v) => onChange({ ...date, month: parseInt(v) })}>
      ...
      <div className="space-y-2">
        ...
        <Select value={date.day.toString()} onValueChange={(v) => onChange({ ...date, day: parseInt(v) })}>
