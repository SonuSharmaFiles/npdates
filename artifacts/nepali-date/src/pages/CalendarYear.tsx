...
export default function CalendarYear() {
  const [, params] = useRoute("/nepali-calendar-:year");
  const year = Number(params?.year);
  if (!year || year < BS_MIN_YEAR || year > BS_MAX_YEAR) {
    return (
      <div className="container mx-auto px-4 py-16">
        <h1 className="font-serif text-3xl font-bold">Year out of range</h1>
        <p className="text-muted-foreground mt-2">
          Please choose a Nepali year between {BS_MIN_YEAR} and {BS_MAX_YEAR}.
        </p>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <header className="mt-6 mb-10">
          ...
          <p className="text-muted-foreground mt-3 text-lg">
            ...
            <strong>{yearStart.ad.formatted}</strong>. {fests.length} festivals
            and public holidays tracked.
          </p>
        </header>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
          {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
            <Link
              ...
              className="block hover-elevate-2 rounded-2xl"
            >
        ...
        <section className="mt-16 max-w-3xl prose prose-neutral dark:prose-invert">
          <h2>About BS {year}</h2>
          <p>
            Bikram Sambat year <strong>{year}</strong> begins on{" "}
            {yearStart.ad.formatted} ({yearStart.ad.weekdayName}) and runs through
            the following spring. The 12 Nepali months in order are{" "}
            {BS_MONTHS_EN.map((m, i) => `${m} (${BS_MONTHS_NP[i]})`).join(", ")}.
          </p>
