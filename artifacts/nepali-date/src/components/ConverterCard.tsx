...
import { bsToAd, adToBs, getTodayInKathmandu, ConversionResult, toNepaliNumeral } from "@/lib/converter";
...
export function ConverterCard({ initialDirection = "BS_TO_AD" }: { initialDirection?: Direction }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("nd:history");
    if (saved) {
      try {
  ...
  useEffect(() => {
    try {
      if (direction === "BS_TO_AD") {
        const res = bsToAd(bsDate.year, bsDate.month, bsDate.day);
        setResult(res);
      } else {
        const res = adToBs(adDate.year, adDate.month, adDate.day);
        setResult(res);
      }
    } catch (e) {
      setResult(null);
  ...
  const handleNavigate = () => {
    ...
    if (direction === "BS_TO_AD") {
      ...
      setLocation(`/bs-to-ad/${result.bs.year}/${result.bs.month}/${result.bs.day}`);
    } else {
      addToHistory(result, direction);
      setLocation(`/ad-to-bs/${result.ad.year}/${result.ad.month}/${result.ad.day}`);
    }
  };
        ...
        <div className="mt-6 bg-muted/30 rounded-xl p-6 border text-center relative overflow-hidden group">
          {result ? (
            ...
            <div className="py-8 text-muted-foreground">
              Invalid date combination. Please adjust your selection.
            </div>
          )}
        </div>
