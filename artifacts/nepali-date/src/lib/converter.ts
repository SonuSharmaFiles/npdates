...
export function bsToAd(year: number, month: number, day: number): ConversionResult {
  ...
  for (let y = BS_MIN_YEAR; y < year; y++) {
    ...
    if (!data) {
      throw new Error(`Missing BS calendar data for year ${y}.`);
    }
...
export function getFiscalYear(startBsYear: number): FiscalYearWindow {
  // Nepal fiscal year runs from 1 Shrawan (month 4) to end of Ashadh (month 3) next year.
  ...
  const shortStart = String(startBsYear).slice(-2);
...
export function ageFromAd(birthIso: string, asOfIso?: string): {
  ...
  return {
    ...
    months,
    days,
    totalDays: daysBetweenAdDates(birth, now),
  };
}
